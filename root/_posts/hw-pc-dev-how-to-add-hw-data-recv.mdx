...
-   기타 다양하게 데이터를 파싱해야 할 수 있습니다.
-   권장하는 파싱 방법은 파서를 만드는 것입니다.
-   시리얼 포트로부터 전달된 데이터 스트림에 파서를 연결하여
    파서로부터 하나씩 꺼내서 데이터를 처리하는 방식입니다.
-   NodeJS의 스트림 API를 활용합니다.
...
그리고 현재 PC 프로그램에 구현된 다른 하드웨어들을 참고하시면 좋을 것 같습니다.
...
위의 예제에서는 `includeDelimiter:false`로 설정했으므로, 패킷을 읽을 때 구분 문자는 제거하고 읽어집니다.
...
-   `this.readNext_()` 함수는 `WiseXboardControl` 클래스가 상속하고 있는 `AbstractHwConrtol` 클래스에 구현되어 있습니다.
    따라서 다른 하드웨어에서도 사용되는 함수입니다.
-   `this.readNext_()` 함수는 내부적으로, `openDevice()` 함수가 리턴한 SerialDevice 객체를 이용해서 시리얼 포트로부터 값을 읽습니다.

### saeonAltinoLite

이번엔 `saeonAltinoLite` 하드웨어를 살펴보겠습니다.

-   `saeonAltinoLite`는 자체 제작한 파서를 사용하고 있습니다.
-   아래 소스코드에서 `AltinoLiteParser` 부분을 살펴보세요.
...
커스텀하게 직접 구현한 파서인 `AltinoLiteParser`의 소스코드가 길지 않으므로 여기에 적어보겠습니다.

-   커스텀 파서는 `Transform` 클래스를 상속해서 `_transform()` 함수를 잘 구현하면 됩니다.
...
-   `serialport` 사이트에서 제공하는 여러 파서들을 활용하세요.
